...
export async function generateMetadata({ params }: { params: { locale: string; offer_slug: string } }): Promise<Metadata> {
  const item = await getOffer(params.offer_slug, params.locale);
  if (!item) return { title: 'Not Found' };
  return buildMetadata({
    entity: item,
    locale: params.locale,
    path: `/offer/${params.offer_slug}`,
    fallbackTitle: item.title,
    ogImage: item.og_image ? storageUrl(item.og_image) : item.image ? storageUrl(item.image) : null,
  });
}

export default async function OfferDetailPage({ params }: { params: { locale: string; offer_slug: string } }) {
  const { locale, offer_slug } = params;
  const t = getTranslations(locale);
  const item = await getOffer(offer_slug, locale);
  if (!item) notFound();
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      <Breadcrumbs items={[{ label: t.nav.home, href: `/${locale}` }, { label: t.nav.offers, href: `/${locale}/offers` }, { label: item.title }]} />
