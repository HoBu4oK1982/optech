...
// ID остаётся здесь запасным вариантом: поле yandex_metrika_id в настройках
// админки пока пустое, и без фолбэка счётчик просто перестал бы работать.
const FALLBACK_METRIKA_ID = 110481428;
...
function MetrikaHitTracker({ id }: { id: number }) {
...
export default function YandexMetrika({ id }: { id?: string | number | null }) {
  const metrikaId = Number(id) || FALLBACK_METRIKA_ID;
  ...
  return (
    <>
      ...
      <Suspense fallback={null}>
        <MetrikaHitTracker id={metrikaId} />
      </Suspense>
    </>
  );
}
