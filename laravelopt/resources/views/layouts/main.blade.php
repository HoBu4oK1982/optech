...
<body>
    <div class="ad-admin-shell">
        ...
        <aside class="ad-sidebar">
            ...
            <nav class="ad-nav" id="adMainNav">
                <a wire:navigate class="ad-nav-link" data-match="/dashboard" href="{{ route('dashboard') }}"><span class="ad-nav-icon"><i class="fas fa-th-large"></i></span><span>Дашборд</span></a>
                <a wire:navigate class="ad-nav-link" data-match="/categor" href="{{ route('categories') }}"><span class="ad-nav-icon"><i class="fas fa-sitemap"></i></span><span>Категории</span></a>
                <a wire:navigate class="ad-nav-link" data-match="/product" href="{{ route('products') }}"><span class="ad-nav-icon"><i class="fas fa-box"></i></span><span>Товары</span></a>
                <a wire:navigate class="ad-nav-link" data-match="/brand" href="{{ route('brands') }}"><span class="ad-nav-icon"><i class="fas fa-tag"></i></span><span>Бренды</span></a>
            </nav>

            <div class="ad-nav-label">Контент</div>
            <nav class="ad-nav">
                <a wire:navigate class="ad-nav-link" data-match="/article" href="{{ route('articles') }}"><span class="ad-nav-icon"><i class="fas fa-newspaper"></i></span><span>Статьи</span></a>
                <a wire:navigate class="ad-nav-link" data-match="/solcategor" href="{{ route('solcategories') }}"><span class="ad-nav-icon"><i class="fas fa-cubes"></i></span><span>Категории решений</span></a>
                <a wire:navigate class="ad-nav-link" data-match="/solution" href="{{ route('solutions') }}"><span class="ad-nav-icon"><i class="fas fa-layer-group"></i></span><span>Решения</span></a>
                <a wire:navigate class="ad-nav-link" data-match="/project" href="{{ route('projects') }}"><span class="ad-nav-icon"><i class="fas fa-briefcase"></i></span><span>Проекты</span></a>
                <a wire:navigate class="ad-nav-link" data-match="/service" href="{{ route('services') }}"><span class="ad-nav-icon"><i class="fas fa-cogs"></i></span><span>Услуги</span></a>
                <a wire:navigate class="ad-nav-link" data-match="/offer" href="{{ route('offers') }}"><span class="ad-nav-icon"><i class="fas fa-percent"></i></span><span>Спецпредложения</span></a>
            </nav>

            <div class="ad-nav-label">Сайт</div>
            <nav class="ad-nav">
                <a wire:navigate class="ad-nav-link" data-match="/slider" href="{{ route('sliders') }}"><span class="ad-nav-icon"><i class="fas fa-images"></i></span><span>Слайдеры</span></a>
                <a wire:navigate class="ad-nav-link" data-match="/partner" href="{{ route('partners') }}"><span class="ad-nav-icon"><i class="fas fa-handshake"></i></span><span>Партнёры</span></a>
                <a wire:navigate class="ad-nav-link" data-match="/license" href="{{ route('licenses') }}"><span class="ad-nav-icon"><i class="fas fa-certificate"></i></span><span>Лицензии</span></a>
                <a wire:navigate class="ad-nav-link" data-match="/setting" href="{{ route('settings') }}"><span class="ad-nav-icon"><i class="fas fa-sliders-h"></i></span><span>Настройки и SEO</span></a>
                <a wire:navigate class="ad-nav-link" data-match="/redirect" href="{{ route('redirects') }}"><span class="ad-nav-icon"><i class="fas fa-route"></i></span><span>Редиректы</span></a>
            </nav>
        ...
        <main class="ad-main">
            ...
            <section class="ad-content" id="adContent">
                @isset($header)<div class="ad-jet-header" style="margin-bottom:18px;">{{ $header }}</div>@endisset
                {{ $slot ?? '' }}
                @yield('content')
            </section>
    ...
    <script>
        ...
        function updateNavActive() {
            ...
            var links = document.querySelectorAll('.ad-nav-link[data-match]');
            var best = null, bestLen = -1;
            links.forEach(function (l) {
                var m = l.getAttribute('data-match');
                if (m && path.indexOf(m) === 0 && m.length > bestLen) { best = l; bestLen = m.length; }
            });
            links.forEach(function (l) { l.classList.toggle('is-active', l === best); });
        }
        document.addEventListener('click', function (e) {
            var link = e.target.closest('.ad-nav-link[data-match]');
            if (!link) return;
            document.querySelectorAll('.ad-nav-link[data-match]').forEach(function (l) { l.classList.remove('is-active'); });
        ...
        updateNavActive(); applyStoredTheme();
        document.addEventListener('livewire:navigated', function(){ updateNavActive(); applyStoredTheme(); });
        window.addEventListener('popstate', function(){ updateNavActive(); });
    </script>
</body>
</html>
