{{-- Форма решения. Параметр: $action. Доступно: $solCategories, $products, $solutions --}}
<form wire:submit="{{ $action }}">
    <div class="ad-form-actions-top">
        <a wire:navigate href="{{ route('solutions') }}" class="ad-btn"><i class="fas fa-arrow-left"></i> К списку</a>
    ...
    <div class="ad-form-layout">
        <div class="ad-form-main">
            <section class="ad-form-section">
                <div class="ad-section-head"><span class="ad-section-icon"><i class="fas fa-layer-group"></i></span><div><h3>Основное</h3></div></div>
                <div data-langtabs>
                ...
                <div class="ad-fields-grid two" style="margin-top:14px;">
                    ...
                    <label class="ad-field"><span>Категория решений</span><select wire:model="category_id"><option value="">— нет —</option>@foreach($solCategories as $sc)<option value="{{ $sc->id }}">{{ $sc->title_ru }}</option>@endforeach</select></label>
                </div>
            </section>
            ...
            <section class="ad-form-section">
                ...
                <div class="ad-fields-grid two">
                    <div class="ad-field"><span>Главное фото</span><div class="ad-upload"><input type="file" class="ad-upload__input" id="sol-img" wire:model="image" accept="image/*">@if($image)<div class="ad-upload__preview"><img src="{{ $image->temporaryUrl() }}"></div>@elseif(!empty($existing_image))<div class="ad-upload__preview"><img src="{{ asset('assets/images/solutions/' . $existing_image) }}"></div>@endif<label for="sol-img" class="ad-upload__zone"><div class="ad-upload__icon"><i class="fas fa-cloud-upload-alt"></i></div><div class="ad-upload__text"><strong>Загрузить</strong><span>до 5 МБ</span></div></label></div></div>
                    <div class="ad-field"><span>OG-картинка</span><div class="ad-upload"><input type="file" class="ad-upload__input" id="sol-og" wire:model="og_image" accept="image/*">@if($og_image)<div class="ad-upload__preview"><img src="{{ $og_image->temporaryUrl() }}"></div>@elseif(!empty($existing_og_image))<div class="ad-upload__preview"><img src="{{ asset('assets/images/solutions/' . $existing_og_image) }}"></div>@endif<label for="sol-og" class="ad-upload__zone"><div class="ad-upload__icon"><i class="fas fa-share-alt"></i></div><div class="ad-upload__text"><strong>OG</strong><span>необязательно</span></div></label></div></div>
                </div>
                <div class="ad-field" style="margin-top:16px;"><span>Галерея</span>
                    <div class="ad-gallery">
                        @foreach($gallery as $i => $g)<div class="ad-gallery__item" wire:key="g-{{ $i }}"><img src="{{ asset('assets/images/solutions/' . $g) }}"><button type="button" class="ad-gallery__remove" wire:click="removeGallery({{ $i }})"><i class="fas fa-times"></i></button></div>@endforeach
                        @foreach($galleryUploads as $i => $gu)<div class="ad-gallery__item" wire:key="gu-{{ $i }}"><img src="{{ $gu->temporaryUrl() }}"></div>@endforeach
                    </div>
                    <label for="sol-gal" class="ad-upload__zone" style="margin-top:10px;"><div class="ad-upload__icon"><i class="fas fa-images"></i></div><div class="ad-upload__text"><strong>Добавить в галерею</strong><span>несколько файлов</span></div></label>
                    <input type="file" class="ad-upload__input" id="sol-gal" wire:model="galleryUploads" accept="image/*" multiple>
                </div>
            </section>

            <section class="ad-form-section">
                <div class="ad-section-head"><span class="ad-section-icon"><i class="fas fa-link"></i></span><div><h3>Перелинковка</h3><p>Связанные товары, решения и категории.</p></div></div>
                <div class="ad-fields-grid three">
                    ...
                    <label class="ad-field"><span>Категории решений</span><select multiple size="6" wire:model="related_categories">@foreach($solCategories as $sc)<option value="{{ $sc->id }}">{{ $sc->title_ru }}</option>@endforeach</select></label>
                </div>
            </section>
        ...
        <div class="ad-form-side">
            <section class="ad-form-section">
                ...
                <div class="ad-fields-grid" style="margin-top:16px;">
                    <label class="ad-field"><span>Статус</span><select wire:model="status"><option value="0">Опубликовано</option><option value="1">Скрыто</option></select></label>
                    <label class="ad-check-field"><input type="checkbox" wire:model="is_featured"><span>Рекомендуемое (на главной)</span></label>
